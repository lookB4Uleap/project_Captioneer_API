...
# , static_url_path = "/app/static", static_folder = "static"
app = Flask(__name__)
...
def generate_caption():
    if request.method == 'POST':
        ...
        vocab = np.load('static/models/vocab.npy', allow_pickle=True)
        vocab = vocab.item()
        ...
        model = load_model('static/models/image_caption_generator_model.h5')
        model.load_weights('static/models/model_weights.h5')

        resnet = load_model('static/models/resnet50.h5')
        ...
        f.save(folder + f.filename)
        # print(f)
        
        image = cv2.imread(folder + str(f.filename))
